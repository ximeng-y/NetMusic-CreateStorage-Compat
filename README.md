...
[NetMusic](https://github.com/TartaricAcid/NetMusic) 与 [Create:Storage](https://github.com/ausmez/create-storage-neo-forge) 的兼容修复 mod：让 NetMusic 的音乐唱片（`music_cd`）可以放入 Create:Storage 背包的唱片机升级（JukeboxUpgrade）槽位并正常播放，充当随身音响。
...
A compatibility fix mod for [NetMusic](https://github.com/TartaricAcid/NetMusic) and [Create:Storage](https://github.com/ausmez/create-storage-neo-forge): it allows NetMusic music discs (`music_cd`) to be placed into the JukeboxUpgrade slot of Create:Storage backpacks and play normally, turning the backpack into a portable music player.
